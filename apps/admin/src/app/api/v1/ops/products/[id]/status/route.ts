...
import { ProductStatusSchema } from '@validators/ops/product';
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = ProductStatusSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    await ProductService.setStatus(id, parsed.isActive, payload.userId);
