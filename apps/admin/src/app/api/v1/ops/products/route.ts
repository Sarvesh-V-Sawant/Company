...
import { ListProductsSchema, CreateProductSchema } from '@validators/ops/product';
import { ProductService } from '@services/ops/ProductService';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  let query;
  try { query = ListProductsSchema.parse(Object.fromEntries(searchParams)); }
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  let parsed;
  try { parsed = CreateProductSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await ProductService.create(parsed, payload.userId);
