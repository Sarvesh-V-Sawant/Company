...
import { ListAddressesSchema, CreateAddressSchema } from '@validators/ops/address';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  let query;
  try { query = ListAddressesSchema.parse(Object.fromEntries(searchParams)); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await AddressService.list(query);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  let parsed;
  try { parsed = CreateAddressSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await AddressService.create(parsed, payload.userId);
