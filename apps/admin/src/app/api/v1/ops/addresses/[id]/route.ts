...
import { UpdateAddressSchema } from '@validators/ops/address';
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = UpdateAddressSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await AddressService.update(id, parsed, payload.userId);
    return success(result);
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.message, err.httpStatus);
    throw err;
