...
import { ListManufacturersSchema, CreateManufacturerSchema } from '@validators/ops/manufacturer';
import { ManufacturerService } from '@services/ops/ManufacturerService';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  let query;
  try { query = ListManufacturersSchema.parse(Object.fromEntries(searchParams)); }
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  let parsed;
  try { parsed = CreateManufacturerSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await ManufacturerService.create(parsed, payload.userId);
