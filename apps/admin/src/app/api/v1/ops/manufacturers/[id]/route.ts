...
import { UpdateManufacturerSchema } from '@validators/ops/manufacturer';
...
export async function GET(
  ...
  try {
    const result = await ManufacturerService.getById(id);
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = UpdateManufacturerSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await ManufacturerService.update(id, parsed, payload.userId);
    return success(result);
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.message, err.httpStatus);
    throw err;
