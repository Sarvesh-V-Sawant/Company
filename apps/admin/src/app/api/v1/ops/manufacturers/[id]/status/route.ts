...
import { ManufacturerStatusSchema } from '@validators/ops/manufacturer';
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = ManufacturerStatusSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    await ManufacturerService.setStatus(id, parsed.isActive, payload.userId);
