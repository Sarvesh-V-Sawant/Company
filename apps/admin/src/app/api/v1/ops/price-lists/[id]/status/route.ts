...
import { PriceListStatusSchema } from '@validators/ops/price-list';
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = PriceListStatusSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    await PriceListService.setStatus(id, parsed.isActive, payload.userId);
