...
import { CanteenStatusSchema } from '@validators/ops/canteen';
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = CanteenStatusSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    await CanteenService.setStatus(id, parsed.isActive, payload.userId);
