...
import { UpdateCanteenSchema } from '@validators/ops/canteen';
...
export async function GET(
  ...
  try {
    const result = await CanteenService.getById(id);
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = UpdateCanteenSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await CanteenService.update(id, parsed, payload.userId);
    return success(result);
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.message, err.httpStatus);
    throw err;
