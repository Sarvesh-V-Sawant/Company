...
import { UpdateCommissionRuleSchema } from '@validators/ops/commission-rule';
...
export async function GET(
  ...
  try {
    const result = await CommissionRuleService.getById(id);
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = UpdateCommissionRuleSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    const result = await CommissionRuleService.update(id, parsed, payload.userId);
    return success(result);
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.message, err.httpStatus);
    throw err;
