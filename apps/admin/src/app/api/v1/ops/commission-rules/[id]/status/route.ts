...
import { CommissionRuleStatusSchema } from '@validators/ops/commission-rule';
...
export async function PATCH(
  ...
  let parsed;
  try { parsed = CommissionRuleStatusSchema.parse(body); }
  catch (err) { if (err instanceof ZodError) return apiError('GEN_001', 'Validation failed.', 400); throw err; }

  try {
    await CommissionRuleService.setStatus(id, parsed.isActive, payload.userId);
