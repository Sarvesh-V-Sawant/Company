...
import { AttendanceWeeklyQuerySchema } from '@validators/attendance';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  let query;
  try {
    query = AttendanceWeeklyQuerySchema.parse(Object.fromEntries(searchParams));
  } catch (err) {
  ...
  try {
    const result = await AttendanceService.getWeekly(targetEmployeeId, query.week);
