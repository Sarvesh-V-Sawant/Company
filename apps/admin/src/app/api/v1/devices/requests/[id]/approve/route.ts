...
import { ApproveDeviceRequestSchema } from '@validators/device';
...
export async function PATCH(
  ...
  let parsed;
  try {
    parsed = ApproveDeviceRequestSchema.parse(body);
  } catch (err) {
  ...
  try {
    const result = await DeviceService.approveRequest(id, payload.userId, parsed.approvalNote);
