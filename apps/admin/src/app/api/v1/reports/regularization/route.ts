...
import { RegularizationReportQuerySchema } from '@validators/report';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  let query;
  try { query = RegularizationReportQuerySchema.parse(sp); }
  catch (err) {
  ...
  try {
    const result = await ReportService.regularizationReport(payload.userId, query);
