...
import { UpdateGeofenceSchema } from '@validators/settings';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return success({ geoFence: s.geoFence }, 200);
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.message, err.httpStatus);
    throw err;
  }
...
export async function PATCH(request: NextRequest): Promise<NextResponse> {
  ...
  let parsed;
  try { parsed = UpdateGeofenceSchema.parse(body); }
  catch (err) {
  ...
  try {
    const s = await SettingsService.updateGeofence(parsed);
    return success({ geoFence: s.geoFence }, 200);
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.message, err.httpStatus);
    throw err;
